...
class User:
    ...
    def login(self, username, password):
        if self.is_loged_in:
            return False
        response = requests.post(settings.LOGIN_ENDPOINT_URL, data={
    ...
    def register(self, username, password):
        response = requests.post(settings.REGISTER_ENDPOINT_URL, data={
                                 "username": username, "password": password})
    ...
    def get_user_statistics(self):
        ...
        response = requests.get(settings.STATISTICS_ENDPOINT_URL, headers={
    ...
    def deposit(self, amount):
        ...
        response = requests.post(settings.DEPOSIT_ENDPOINT_URL, headers={
                                 "Authorization": self.get_authorization_header()}, data={"amount": amount})
        json_data = response.json()
