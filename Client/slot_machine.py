...
class SlotMachine:
    def __init__(self, user_interface_bg, user):
        ...
        self.user = user
        self.start_spin_sound = pygame.mixer.Sound(
            settings.SPIN_START_SOUND_PATH)
    ...
    def initialize_reels(self):
        for i in range(5):
            width = i*(settings.IMAGE_WIDTH +
                       settings.WIDTH_OFFSET) + settings.START_WIDTH
    ...
    def vizualize_winnings(self):
        ...
        img = font.render(
            f"balance:{round(self.user.balance,2)}", True, (255, 255, 255))

        self.display_surface.blit(
            self.user_interface_bg, (0, settings.SCREEN_HEIGHT))
    ...
    def get_spin_result(self, amount):
        response = requests.post(settings.SPIN_REQUST_ENDPOINT_URL, headers={
